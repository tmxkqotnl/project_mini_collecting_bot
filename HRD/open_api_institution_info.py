...
def get_institution_info(
    ...
    if info_detail == "default":
        func_type = parse_institution_info
    elif info_detail == "facility_detail":
        func_type = parse_facility_info
    elif info_detail == "eqnm_detail":
        func_type = parse_inst_eqpm_info
    else:
        raise ValueError("No case for info_type - {}".format(info_detail))

    return pd.DataFrame(func_type(xml_res))
...
def parse_facility_info(
    ...
    inst_facility_info_list = xml.find_all("inst_eqnm_info_list")
